  ...
  <v-card class="line-card pa-1 rounded-lg d-flex flex-column flex-grow-1" elevation="5" outlined
    ...
    <v-card-title class="card-title pa-3 text-uppercase flex-shrink-1 justify-space-between"
      ...
      <div style="width: max-content;">
        <SelectCategory :controls_point="controls_points" :control_value="control_value"
          @change_endpoint="select_changed_control" />
      </div>
      ...
      <div v-if="next && prev" style="height: 40px;">
        ...
        <v-btn @click="$emit('nav', +1)"
          style="font-size: 14px !important; border-radius: 10px;  min-width: 36px !important; box-shadow: none;">{{
            next
    ...
    <div class="d-flex flex-column flex-grow-1 flex-shrink-1" style="height:0; position: relative;">
      <v-progress-circular style="position: absolute;left: 53.5%; top: 8px; transform: translateX(-50%);"
        v-if="lineChartData.datasets.length == 1" :size="25" color="#14202c" indeterminate />
...
export default {
  ...
  mounted() {
    this.$store.state.appDataStore.appConfig.config.forEach((element) => {
