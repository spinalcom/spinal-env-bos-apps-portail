...
<template>
  <v-app v-if="pageSate === PAGE_STATES.loaded" class="app">
    ...
    <div class="dataBody">
      ...
      <dataSideApp v-if="config" :DActive="isActive3D" :ActiveData="isActive" :class="{ 'active': isActive, 'inactive': isActive3D }"
...
class App extends Vue {
  ...
  async mounted() {
    ...
    if (!currentQuery.spaceSelectedId) {
      console.log('NO QUERY AVAILABLE, LOADING DEFAULT BUILDING VIEW')
