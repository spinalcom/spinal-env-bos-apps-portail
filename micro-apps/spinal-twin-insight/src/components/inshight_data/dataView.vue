...
<template>
  <div class="dataView" :id="item.dynamicId" :class="{ subItem: !isTitle, isSelected: isSelected() }"
    ...
    <v-icon size="20" v-if="is_controllable" style="padding-right: 5px;"
      @click.stop="EditEndpoint(item)">mdi-square-edit-outline</v-icon>
    <div v-if="isChartPossible">
      <v-btn icon x-small @click.stop="clickChart">
...
export default {
  ...
  async mounted() {
    ...
    const vm = this;
    const sources = store.state.appDataStore.appConfig.source;
    let source = {}
