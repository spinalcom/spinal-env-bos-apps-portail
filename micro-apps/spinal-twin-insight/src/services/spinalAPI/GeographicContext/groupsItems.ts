...
export async function getGroupsItems(
  config: IConfig,
  buildingId: string
...
export async function getAllCategoriesTree(
  ...
  if (!contexts)
    throw new Error(
      `[regroupment] - No group context found for ${contextName}`
    );

  contexts = Array.isArray(contexts) ? contexts : [contexts];
  const geoGroupContexts = contexts.filter(
    (el) =>
      ["BIMObjectGroupContext", "geographicRoomGroupContext"].indexOf(
        el.type
      ) !== -1
  );
  const promises = geoGroupContexts.map(async (el) =>
    _getTree(el.dynamicId, buildingId)
  );
...
async function getGroupContextTree(
  buildingId: string,
  contextName
): Promise<INodeItem[]> {
  ...
  const url = spinalAPI.createUrlWithPlatformId(
    buildingId,
    `/api/v1/groupContext/${context.dynamicId}/tree`
  );
  let result = await spinalAPI.get<any>(url);
  return result.data;
}

async function getGroupContextTreeByNames(
  buildingId: string,
  ...
  (context as any).children = await getGroupTreeFromCategory(
    buildingId,
    context.dynamicId,
    entryPoint
  );
...
async function getGroupTreeFromCategory(
  buildingId: string,
  contextId: number,
  entryPoint: EntryPoint
) {
  const categories = await _getCategoriesRequest(
    contextId,
    buildingId,
    entryPoint.category
  );
  ...
  const promises = categories.map(async (el) => {
    (el as any).children = await getGroupTreeFromGroup(
      buildingId,
      contextId,
      el.dynamicId,
      entryPoint
...
async function getGroupTreeFromGroup(
  buildingId: string,
  contextId: number,
  categoryId: number,
  entryPoint: EntryPoint
) {
  const groups = await _getGroupsRequest(
    contextId,
    categoryId,
    buildingId,
    entryPoint.group
  ...
  const promises = groups.map(async (el) => {
    (el as any).children = await _getGroupsItemsRequest(
      buildingId,
      contextId,
      categoryId,
      el
...
async function classifyItemsByGeographicTypes(
  buildingId: string,
  items: any[]
) {
  const { dynamicIds, type, obj } = items.reduce(
    (data, item) => {
  ...
  const positions = await getPositionMultiple(
    buildingId,
    dynamicIds,
    type,
    200
...
function _createCopyWithoutChildren(objToCopy: any, parent: any) {
  ...
  for (const key in objToCopy) {
    if (
      Object.prototype.hasOwnProperty.call(objToCopy, key) &&
      key !== "children"
    ) {
...
async function _getTree(contextId: number, buildingId: string) {
  ...
  const url = spinalAPI.createUrlWithPlatformId(
    buildingId,
    `/api/v1/context/${contextId}/tree/2/depth`
  );
  let result = await spinalAPI.get<INodeItem[]>(url);
  return result.data;
}

async function _getCategoriesRequest(
  contextId: number,
  buildingId: string,
  categoryName?: string
) {
  ...
  const url = spinalAPI.createUrlWithPlatformId(
    buildingId,
    `/api/v1/groupeContext/${contextId}/category_list`
  );
  let result = await spinalAPI.get<INodeItem[]>(url);
...
async function _getGroupsRequest(
  contextId: number,
  categoryId: number,
  buildingId: string,
  groupName?: string
): Promise<(INodeItem & { categoryId: number; contextId: number })[]> {
  const spinalAPI = SpinalAPI.getInstance();
  const url = spinalAPI.createUrlWithPlatformId(
    buildingId,
  ...
  return result.data.reduce(
    (
      list: (INodeItem & { categoryId: number; contextId: number })[],
      el,
      index,
      arr
    ) => {
...
async function _getGroupsItemsRequest(
  buildingId: string,
  contextId: number,
  categoryId: number,
  group: INodeItem
  ...
  let endpoint;
  if (group.type === "geographicRoomGroup")
    endpoint = `/api/v1/roomsGroup/${contextId}/category/${categoryId}/group/${group.dynamicId}/roomList`;
  if (group.type === "BIMObjectGroup")
    endpoint = `/api/v1/equipementsGroup/${contextId}/category/${categoryId}/group/${group.dynamicId}/equipementList`;
...
function getPositionMultiple(
  ...
  if (type === "geographicRoom")
    apiRoute = "/api/v1/room/get_position_multiple";

  const promises = lodash
    .chunk(dynamicIds, size)
    .map((ids) => getPositionMultipleRequest(buildingId, ids, apiRoute));
...
async function getPositionMultipleRequest(
  buildingId: string,
  dynamicIds: string[],
  apiRoute: string
) {
