...
<template>
  <v-app id="application">
    ...
    <div style="width: 100%" class="d-flex justify-end">
      <sc-download-button :file-name="'Tickets'" :data="downloadList"></sc-download-button>
      <div class="Hx1 mx-2">
        <space-selector ref="space-selector" :open.sync="openSpaceSelector" :maxDepth="0" :GetChildrenFct="expand"
          v-model="el" label="ESPACE" />
      </div>
    </div>

    <!--Ticket table-->
    <v-card class="ticket-table ma-2 d-flex flex-column table-card rounded-lg flex-grow-1" elevation="5" outlined>
      <v-card-title style="height: 56px" class="text-uppercase ma-2">Liste des tickets</v-card-title>
      <div ref="tableContainer" style="height: calc(100% - 56px)" class="d-flex flex-column">
        <div ref="dataTable" class="d-flex ml-6 mr-6 mb-6 flex-fill">
          <v-data-table item-key="name"
            ...
            loading-text="Chargement des données" :loading="!loaded" fixed-header :headers="headers"
            :height="tableHeight" :items="stepFilteredTickets" :footer-props="{
              prevIcon: 'mdi-menu-left',
              nextIcon: 'mdi-menu-right',
              showCurrentPage: true,
              itemsPerPageAllText: 'Tout',
            }" @click:row="rowClicked">
            <v-progress-linear v-show="!loaded" slot="progress" color="accent" class="progress-bar" indeterminate
              style="margin-left: -16px; width: calc(100% + 32px)"></v-progress-linear>
            <template v-slot:no-data> Pas de données disponibles </template>
            <template v-slot:header.Domaine="{ header }">
              <v-select color="#14202C" style="
                  width: calc(100% - 18px);
                  margin-left: -12px;
                  transform: translate(0%, 4%);
                " class="d-inline-block mt-4" dense solo flat @click.stop v-model="domain_filter" label="Domaine"
                placeholder="Domaine" :items="domains" append-icon="mdi-chevron-down" clearable
            ...
            <template v-slot:header.Étape="{ header }">
              <v-select color="#14202C" style="
                  ...
                  margin-left: -12px;
                  transform: translate(0%, 4%);
                " class="d-inline-block mt-4" dense solo flat @click.stop v-model="step_filter" label="Étape"
                placeholder="Étape" :items="steps" append-icon="mdi-chevron-down" clearable
                clear-icon="mdi-close-circle-outline" multiple menu-props="offset-y">
                <template v-slot:selection="{ item, index }">
                  <v-chip @click:close="
    ...
    <sc-ticket-detail v-if="detailedTicket" v-model="showDialog" :detailedTicket="detailedTicket" :baseURL="baseURL"
      :token="token"></sc-ticket-detail>
  </v-app>
</template>
...
import { displayDate } from "./services/store/appDataStore/utils";
...
export default {
  ...
  computed: {
    ...
    tableData() {
      return this.loaded ? this.$store.getters['getTickets'](this.el.dynamicId): [];
    },
    detailedTicket() {

      return this.$store.state.appDataStore.tickets.find((t) => t.dynamicId === this.selectedId);
    },
    headers() {
    ...
    downloadList() {
      ...
      return this.stepFilteredTickets.map((t) => {
        ...
        const tdetail = this.$store.state.appDataStore.tickets.find((tc) => tc.dynamicId === t.id);
        return {
          Nom,
  ...
  methods: {
    ...
    async expand(item) {
      if (item) {
        switch (item.level) {
          case 0:
            const floors = await this.$store.dispatch('getFloorList');
            console.log(floors);
      ...
      building.count = this.$store.state.appDataStore.tickets.length;
      return [building];
    },
  ...
  watch: {
    async el(v) {
      ...
      this.loaded = false;
      await this.$store.dispatch('setFloorTickets', v.dynamicId);
      this.loaded = true;
    },
  },
};
