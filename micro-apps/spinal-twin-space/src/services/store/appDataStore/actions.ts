...
import { postRoomInventory,postFloorInventory,getEquipments, getFloors, getRooms, getBuilding ,
	...
	getNotes, getNodeEndpointList, getNodeControlEndpointList ,getTimeSeriesAsync , getFile, getNodeReadMultiple, getEquipementPositions , getRoomPositions,
...
import { getRoomContexts , getRoomList } from "../../spinalAPI/RoomsGroup/roomsGroup";
...
export const actions = {
	...
	async [ActionTypes.GET_ROOM_GROUP_CONTEXT]({ commit }: AugmentedActionContextAppData, { buildingId, patrimoineId, floorId, id, forceUpdate }: any): Promise<IZoneItem[]> {

		const spinalAPI = SpinalAPI.getInstance();
		if (typeof ApiIteratorStore[ActionTypes.GET_ROOM_GROUP_CONTEXT] === "undefined") {
			ApiIteratorStore[ActionTypes.GET_ROOM_GROUP_CONTEXT] = {};
		}
		const floorObjStore = ApiIteratorStore[ActionTypes.GET_ROOM_GROUP_CONTEXT]!;

		if (typeof floorObjStore[id] === "undefined" || forceUpdate === true) {
			floorObjStore[id] = spinalAPI.createIteratorCall(getRoomContexts, patrimoineId, buildingId, floorId);
		}
		const floors = await floorObjStore[id].next();
		// commit(MutationTypes.SET_DATA, { id: id, items: floors.value });
		return floors.value;
	...
	async [ActionTypes.GET_ROOM_LIST]({ commit }: AugmentedActionContextAppData, { buildingId, patrimoineId, contextDynId, categoryDynId, groupDynId, id, forceUpdate }: any): Promise<IZoneItem[]> {
		const spinalAPI = SpinalAPI.getInstance();
		if (typeof ApiIteratorStore[ActionTypes.GET_ROOM_LIST] === "undefined") {
			ApiIteratorStore[ActionTypes.GET_ROOM_LIST] = {};
		}
		const floorObjStore = ApiIteratorStore[ActionTypes.GET_ROOM_LIST]!;

		if (typeof floorObjStore[id] === "undefined" || forceUpdate === true) {
			floorObjStore[id] = spinalAPI.createIteratorCall(getRoomList, patrimoineId, buildingId, contextDynId, categoryDynId, groupDynId);
		}
		const floors = await floorObjStore[id].next();
		// commit(MutationTypes.SET_DATA, { id: id, items: floors.value });
		return floors.value;
