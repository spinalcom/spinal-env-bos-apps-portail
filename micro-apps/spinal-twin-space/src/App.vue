...
<template>
  <v-app v-if="pageSate === PAGE_STATES.loaded" class="app">
    ...
    <div class="dataBody">
      <viewerApp v-if="config" :class="{ 'active3D': isActive3D }" class="viewerContainer"></viewerApp>
      <dataSideApp v-if="config" :DActive="isActive3D" :ActiveData="isActive"
        :class="{ 'active': isActive, 'inactive': isActive3D }" :selected_attr="$store.state.appDataStore.attr"
        class="appContainer" :element_clicked="el_clicked" :config="config" :selectedZone="selectedZone"
        :tableData="displayedData" @clickOnDataView="onDataViewClicked" @fit-to-view="fitToView"
...
class App extends Vue {
  ...
  query: { config: string; app: string; mode: string; name: string; spaceSelectedId: string; spaceSelectedType: string; buildingId: string } = {
    app: '',
    mode: 'null',
    name: '',
    spaceSelectedId: '',
