...
export async function getGeographicItemsGroups(): Promise<{ [key: string]: INodeItem }> {
	const contexts = await getAllGeographicGroups();
	const trees = contexts.map(async ({ dynamicId, type }) => ({ type: type.replace("GroupContext", ""), data: await getGroupTree(dynamicId) }));
...
async function getAllGeographicGroups(): Promise<INodeItem[]> {
...
async function getGroupTree(dynamicId: number): Promise<{ any }> {
	const spinalAPI = SpinalAPI.getInstance();
	const url = spinalAPI.createUrl(`/api/v1/groupContext/${dynamicId}/tree`);
