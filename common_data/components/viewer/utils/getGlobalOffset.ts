...
export async function getGlobalOffset(viewer: Autodesk.Viewing.Viewer3D): Promise<THREE.Vector3> {
	if (!globalOffset) {
		const scene = await getDefaultOrFirstScene();
		...
		const path = getAPINormalisePath(firstBim.aecPath);
		const aecModelData = await getAecModelData(path);
