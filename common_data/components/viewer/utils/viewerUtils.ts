...
export class ViewerUtils {
	...
	public async load3DModels(viewer: Autodesk.Viewing.Viewer3D, data: IStartLoadModel): Promise<Autodesk.Viewing.Model[]> {
		...
		return load1stThenAll(
			...
			async (d: IloadModelTask): Promise<Autodesk.Viewing.Model> => {
				return this._loadBimFile(viewer, data.loadingType as any, d);
			},
			this._isFirstModel
	...
	private async _loadBimFile(viewer: Autodesk.Viewing.Viewer3D,sceneAlignMethod: number, modelData : IloadModelTask): Promise<Autodesk.Viewing.Model | void> {
		try {
			const option: {
			...
			if(modelData.offset) {
				...
				option.globalOffset = await getGlobalOffset(viewer);				
			
			} else if (sceneAlignMethod === SceneAlignMethod.ShareCoordinates && modelData.aecPath) {
				option.applyRefPoint = true;
				option.globalOffset = await addOffsetFromAEC(modelData.aecPath, viewer);
			}

			const path = getAPINormalisePath(modelData.path);
