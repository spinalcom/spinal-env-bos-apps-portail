...
export class ViewerManager {
	...
	public async loadInViewer(item: IPlayload, loadOnlyThisModel: boolean = true, body?: IViewInfoBody & { dbIdsToAdd?: { bimFileId: string; dbIds: number[] }[] }) {
		...
		emitter.once(VIEWER_INITIALIZED, async () => {
			...
			const viewerInfo = await getViewInfoFormatted(res, item);
	...
	public async getViewerInfo(argItem: IPlayload | IPlayload[], body?: IViewInfoBody): Promise<any[]> {
		...
		if (nodeTofetech.length > 0) {	
			...
			const datas = await getViewInfo(body);
	...
	public async colorItems(item: IPlayload | IPlayload[]) {
		const formatted = await this._getAndFormatViewerInfos(item);
		const emitter = EmitterViewerHandler.getInstance();
		emitter.emit(VIEWER_OBJ_COLOR, formatted as any);
	}

	public async addSprites(item: IPlayload | IPlayload[]) {
		const formatted = await this._getAndFormatViewerInfos(item);
		const emitter = EmitterViewerHandler.getInstance();
		emitter.emit(VIEWER_ADD_SPRITE, formatted as any);
	}

	public async addComponentAsSprites(item: IPlayloadWithComponent | IPlayloadWithComponent[], component?: Vue) {
		const formatted = await this._getAndFormatViewerInfos(item, component);
		const emitter = EmitterViewerHandler.getInstance();
	...
	private async _getAndFormatViewerInfos(item: IPlayloadWithComponent | IPlayloadWithComponent[], component?: Vue) {
		item = Array.isArray(item) ? item : [item];
		const data = await this.getViewerInfo(item);
	...
	private async _fctViewerIteract(eventName: keyof ViewerEventWithData, playload: (IPlayload | string) | (IPlayload | string)[], isolateConfig?: any): Promise<any> {
		...
		let data: IViewInfoItemRes[] = await this.getViewerInfoMerged(playload as IPlayload);
